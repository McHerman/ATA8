...
class SysController(config: Configuration) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val scratchIn = Vec(2,new ReadportScratch)
		val readAddr = Vec(2/* FIXME: magic fucking number*/,Decoupled(new Readport(UInt(c.addrWidth.W), c.tagWidth)))
...
/* 	io.scratchIn(0).request.valid := false.B
	...
	io.readAddr(0).valid := false.B
	io.readAddr(0).bits := DontCare

	io.readAddr(1).valid := false.B
	io.readAddr(1).bits := DontCare
	...
	opbuffer.io.ReadData.ready := false.B
  ...
  val StateReg = RegInit(0.U(4.W))
	...
	switch(StateReg){
		...
		is(1.U){
			io.readAddr.zipWithIndex.foreach { case (element,i) => element.valid := true.B; element.bits.addr := reg.ids(i).tag}

			when(SysDMA.io.in.ready && SysDMA2.io.in.ready){ //TODO: clean up this shit
				SysDMA.io.in.bits.addr := io.readAddr(0).bits.data
				SysDMA.io.in.bits.size := reg.size
				SysDMA2.io.in.bits.addr := io.readAddr(1).bits.data
