...
class Load(config: Configuration) extends Module {
  ...
  val instqueue = Module(new BufferFIFO(16,new LoadInstIssue))
  val LoadController = Module(new LoadController)

  instqueue.io.WriteData <> io.instructionStream
  
  LoadController.io.instructionStream <> instqueue.io.ReadData
  //LoadController.io.AXIST <> io.AXIST
  LoadController.io.AXIData <> io.AXIData
